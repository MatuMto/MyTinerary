...
const authActions = {
   ...
   registerUser: (userToSave)=>{
      return async(dispatch, getState) => {
         const response = await axios.post('http://localhost:4040/api/user/signUp', userToSave)
   ...
   logUser: (incomingUser)=>{
      return async(dispatch, getState)=>{
         const response = await axios.post('http://localhost:4040/api/user/signIn', incomingUser)
   ...
   forcedLoginByLS: (usuarioLS)=>{
      return async(dispatch, getState)=>{
         try {
            const response = await axios.get('http://localhost:4040/api/user/loginLS', {
               headers:{
